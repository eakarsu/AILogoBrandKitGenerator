...
// Route: /api/gap-design
// Feature: Design
// Description: Design version control & rollback
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Design. Design version control & rollback' },
...
router.post('/', async (req, res) => {
  try {
    ...
    return res.json({ feature: 'design', title: 'Design', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'design', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['design']);
      return res.json({ slug: 'design', items: r.rows || [] });
    }
    return res.json({ slug: 'design', items: [] });
  } catch (e) {
    return res.json({ slug: 'design', items: [], error: (e && e.message) || 'Server error' });
  }
});
