...
// Route: /api/gap-ai-brand-name-generator
// Feature: Ai Brand Name Generator
// Description: (creative name suggestions)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Ai Brand Name Generator. (creative name suggestions)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Ai Brand Name Generator\nDescription: (creative name suggestions)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['ai-brand-name-generator', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'ai-brand-name-generator', title: 'Ai Brand Name Generator', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'ai-brand-name-generator', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['ai-brand-name-generator']);
      return res.json({ slug: 'ai-brand-name-generator', items: r.rows || [] });
    }
    return res.json({ slug: 'ai-brand-name-generator', items: [] });
  } catch (e) {
    return res.json({ slug: 'ai-brand-name-generator', items: [], error: (e && e.message) || 'Server error' });
  }
});
