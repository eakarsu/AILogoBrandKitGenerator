...
// Route: /api/gap-ai-generate-logo-image
// Feature: Ai Generate Logo Image
// Description: (actual vector/raster logo synthesis)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Ai Generate Logo Image. (actual vector/raster logo synthesis)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Ai Generate Logo Image\nDescription: (actual vector/raster logo synthesis)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['ai-generate-logo-image', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'ai-generate-logo-image', title: 'Ai Generate Logo Image', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'ai-generate-logo-image', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['ai-generate-logo-image']);
      return res.json({ slug: 'ai-generate-logo-image', items: r.rows || [] });
    }
    return res.json({ slug: 'ai-generate-logo-image', items: [] });
  } catch (e) {
    return res.json({ slug: 'ai-generate-logo-image', items: [], error: (e && e.message) || 'Server error' });
  }
});
