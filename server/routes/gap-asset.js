...
// Route: /api/gap-asset
// Feature: Asset
// Description: Asset export pipeline (PNG/SVG/PDF, various resolutions)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Asset. Asset export pipeline (PNG/SVG/PDF, various resolutions)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    return res.json({ feature: 'asset', title: 'Asset', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'asset', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['asset']);
      return res.json({ slug: 'asset', items: r.rows || [] });
    }
    return res.json({ slug: 'asset', items: [] });
  } catch (e) {
    return res.json({ slug: 'asset', items: [], error: (e && e.message) || 'Server error' });
  }
});
