// Auto-generated feature route: brand-consistency-scan
// Domain: multi-modal brand consistency scanner — Capability: scanning websites and social feeds for brand drift
// Mounted under: /api/brand-consistency-scan
...
// POST /api/brand-consistency-scan/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a multi-modal brand consistency scanner. Your task is scanning websites and social feeds for brand drift. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/brand-consistency-scan/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/brand-consistency-scan/:id — fetch one
...
// DELETE /api/brand-consistency-scan/:id — discard a run
