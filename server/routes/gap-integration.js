...
// Route: /api/gap-integration
// Feature: Integration
// Description: Integration with design tools (Figma, Adobe)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Integration. Integration with design tools (Figma, Adobe)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    return res.json({ feature: 'integration', title: 'Integration', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'integration', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['integration']);
      return res.json({ slug: 'integration', items: r.rows || [] });
    }
    return res.json({ slug: 'integration', items: [] });
  } catch (e) {
    return res.json({ slug: 'integration', items: [], error: (e && e.message) || 'Server error' });
  }
});
