...
// Route: /api/gap-stakeholder
// Feature: Stakeholder
// Description: Stakeholder feedback / approval workflows
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Stakeholder. Stakeholder feedback / approval workflows' },
...
router.post('/', async (req, res) => {
  try {
    ...
    return res.json({ feature: 'stakeholder', title: 'Stakeholder', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'stakeholder', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['stakeholder']);
      return res.json({ slug: 'stakeholder', items: r.rows || [] });
    }
    return res.json({ slug: 'stakeholder', items: [] });
  } catch (e) {
    return res.json({ slug: 'stakeholder', items: [], error: (e && e.message) || 'Server error' });
  }
});
