// Auto-generated feature route: identity-builder-agent
// Domain: agentic brand identity builder — Capability: running conversational strategy → palette + typography + taglines
// Mounted under: /api/identity-builder-agent
  ...
  return new Promise((resolve) => {
    ...
    const options = {
      ...
      headers: {
        ...
        'X-Title': 'Logo Brand Kit Generator'
      }
    };
...
// POST /api/identity-builder-agent/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a agentic brand identity builder. Your task is running conversational strategy → palette + typography + taglines. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/identity-builder-agent/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/identity-builder-agent/:id — fetch one
...
// DELETE /api/identity-builder-agent/:id — discard a run
