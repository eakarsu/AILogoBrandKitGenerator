...
// Route: /api/gap-ai-social-kit-generator
// Feature: Ai Social Kit Generator
// Description: (auto-compose post templates)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Ai Social Kit Generator. (auto-compose post templates)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Ai Social Kit Generator\nDescription: (auto-compose post templates)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['ai-social-kit-generator', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'ai-social-kit-generator', title: 'Ai Social Kit Generator', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'ai-social-kit-generator', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['ai-social-kit-generator']);
      return res.json({ slug: 'ai-social-kit-generator', items: r.rows || [] });
    }
    return res.json({ slug: 'ai-social-kit-generator', items: [] });
  } catch (e) {
    return res.json({ slug: 'ai-social-kit-generator', items: [], error: (e && e.message) || 'Server error' });
  }
});
