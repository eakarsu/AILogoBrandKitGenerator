...
// Route: /api/gap-ai-competitor-positioning
// Feature: Ai Competitor Positioning
// Description: (brand differentiation analysis)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Ai Competitor Positioning. (brand differentiation analysis)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Ai Competitor Positioning\nDescription: (brand differentiation analysis)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['ai-competitor-positioning', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'ai-competitor-positioning', title: 'Ai Competitor Positioning', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'ai-competitor-positioning', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['ai-competitor-positioning']);
      return res.json({ slug: 'ai-competitor-positioning', items: r.rows || [] });
    }
    return res.json({ slug: 'ai-competitor-positioning', items: [] });
  } catch (e) {
    return res.json({ slug: 'ai-competitor-positioning', items: [], error: (e && e.message) || 'Server error' });
  }
});
