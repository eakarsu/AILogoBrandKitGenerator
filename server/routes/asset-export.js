// Auto-generated feature route: asset-export
// Domain: multi-format asset exporter — Capability: producing one-click platform-specific export specs
// Mounted under: /api/asset-export
...
// POST /api/asset-export/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a multi-format asset exporter. Your task is producing one-click platform-specific export specs. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/asset-export/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/asset-export/:id — fetch one
...
// DELETE /api/asset-export/:id — discard a run
