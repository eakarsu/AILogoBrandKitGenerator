...
// Route: /api/gap-brand-guidelines
// Feature: Brand Guidelines
// Description: Brand-guidelines PDF document generation
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Brand Guidelines. Brand-guidelines PDF document generation' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Brand Guidelines\nDescription: Brand-guidelines PDF document generation\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['brand-guidelines', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'brand-guidelines', title: 'Brand Guidelines', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'brand-guidelines', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['brand-guidelines']);
      return res.json({ slug: 'brand-guidelines', items: r.rows || [] });
    }
    return res.json({ slug: 'brand-guidelines', items: [] });
  } catch (e) {
    return res.json({ slug: 'brand-guidelines', items: [], error: (e && e.message) || 'Server error' });
  }
});
