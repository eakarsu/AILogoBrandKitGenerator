// Auto-generated feature route: stakeholder-feedback
// Domain: stakeholder feedback synthesizer — Capability: synthesizing feedback into design iterations
// Mounted under: /api/stakeholder-feedback
...
// POST /api/stakeholder-feedback/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a stakeholder feedback synthesizer. Your task is synthesizing feedback into design iterations. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/stakeholder-feedback/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/stakeholder-feedback/:id — fetch one
...
// DELETE /api/stakeholder-feedback/:id — discard a run
