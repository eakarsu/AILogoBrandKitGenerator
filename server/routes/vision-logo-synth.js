// Auto-generated feature route: vision-logo-synth
// Domain: logo design synthesis agent — Capability: synthesizing logo concepts from briefs or inspiration uploads
// Mounted under: /api/vision-logo-synth
...
// POST /api/vision-logo-synth/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a logo design synthesis agent. Your task is synthesizing logo concepts from briefs or inspiration uploads. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/vision-logo-synth/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/vision-logo-synth/:id — fetch one
...
// DELETE /api/vision-logo-synth/:id — discard a run
