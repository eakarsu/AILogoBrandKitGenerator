...
// Route: /api/gap-limited
// Feature: Limited
// Description: Limited frontend (6 pages vs. 14 backend domains)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Limited. Limited frontend (6 pages vs. 14 backend domains)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    return res.json({ feature: 'limited', title: 'Limited', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'limited', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['limited']);
      return res.json({ slug: 'limited', items: r.rows || [] });
    }
    return res.json({ slug: 'limited', items: [] });
  } catch (e) {
    return res.json({ slug: 'limited', items: [], error: (e && e.message) || 'Server error' });
  }
});
